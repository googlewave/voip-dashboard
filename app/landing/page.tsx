...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-orange-50 font-sans">
      ...
      {/* Sticky Nav */}
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'bg-orange-50 shadow-sm py-4' : 'bg-orange-50 py-6'}`}>
        <div className="max-w-6xl mx-auto px-8 flex items-center justify-between">
          <div className="text-2xl font-extrabold text-stone-800 tracking-tight">🔔 Ring Ring Club</div>
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => router.push('/login')} className="px-5 py-2.5 bg-orange-500 text-white text-sm font-extrabold rounded-lg hover:bg-orange-600 transition shadow-md">
              Join the Club 🔔
            </button>
          </div>
        </div>
      ...
      {/* Hero */}
      <section className="relative pt-36 pb-0 bg-orange-50 overflow-hidden">
        <div className="absolute inset-0 overflow-hidden">
          <div className="absolute -top-20 right-0 w-[500px] h-[500px] bg-orange-100 rounded-full opacity-70 blur-3xl" />
          <div className="absolute bottom-0 -left-20 w-80 h-80 bg-amber-100 rounded-full opacity-60 blur-3xl" />
        </div>
        ...
        <div className="relative max-w-6xl mx-auto px-8 grid grid-cols-1 lg:grid-cols-2 gap-12 items-end">
          ...
          {/* Left: Copy */}
          <div className="pb-20">
            <div className="inline-flex items-center gap-2 bg-orange-100 text-orange-700 text-sm font-semibold px-4 py-2 rounded-full mb-8 border border-orange-200">
              <span className="w-2 h-2 bg-orange-500 rounded-full animate-pulse" />
              Hyper-local. Human. Connected.
            </div>
            <h1 className="text-5xl md:text-6xl font-extrabold text-stone-800 mb-6 leading-tight tracking-tight">
              The phone club your<br />
              <span className="text-orange-500">friends & family deserve.</span>
            </h1>
            <p className="text-stone-500 text-xl mb-10 leading-relaxed max-w-lg">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4">
              <button
                ...
                className="w-full sm:w-auto px-10 py-4 bg-orange-500 text-white text-lg font-extrabold rounded-xl hover:bg-orange-600 transition shadow-lg"
              >
                Join the Club 🔔
              </button>
              <button
                onClick={() => document.getElementById('how')?.scrollIntoView({ behavior: 'smooth' })}
                className="w-full sm:w-auto px-10 py-4 bg-white text-stone-700 text-lg font-semibold rounded-xl hover:bg-orange-100 transition border border-orange-200 shadow-sm"
              >
                See How It Works →
              </button>
            </div>
          ...
          <div className="relative flex items-end justify-center">
            <div className="w-full rounded-t-3xl overflow-hidden shadow-2xl border-4 border-orange-100">
              <img
                src="/hero_split.png"
            ...
            {/* Floating badge bottom right — grandma side */}
            <div className="absolute bottom-8 -right-4 bg-white rounded-2xl shadow-xl p-4 border border-orange-100">
              <div className="flex items-center gap-3">
            ...
            {/* Floating badge top right — grandma side */}
            <div className="absolute top-8 -right-4 bg-white rounded-2xl shadow-xl p-4 border border-orange-100">
              <div className="flex items-center gap-3">
      ...
      {/* Social Proof Bar */}
      <section className="py-10 bg-orange-100 border-y border-orange-200">
        <div className="max-w-6xl mx-auto px-8 text-center">
          <p className="text-orange-400 text-sm font-semibold uppercase tracking-widest mb-6">Families & friend groups sharing the club love</p>
          <div className="flex flex-wrap items-center justify-center gap-10 text-orange-300 font-bold text-base">
            {['The Johnsons', 'Smith Family', 'The Garcias', 'The Williams', 'The Nguyens', 'Team Rodriguez'].map((name) => (
              <span key={name} className="hover:text-orange-500 transition cursor-default">{name}</span>
            ))}
          </div>
        </div>
      </section>
      ...
      <section className="py-28 px-8 bg-white">
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div>
            <span className="text-orange-500 font-semibold text-sm uppercase tracking-widest">Your Circle. Your Club.</span>
            <h2 className="text-4xl md:text-5xl font-extrabold text-stone-800 mt-3 mb-6 leading-tight">
            ...
            <div className="flex flex-wrap gap-4">
              {['Share with neighbors', 'Invite your family', 'Grow your circle', 'Keep it local'].map((tag) => (
                <span key={tag} className="px-4 py-2 bg-orange-50 text-orange-600 font-semibold text-sm rounded-full border border-orange-200">{tag}</span>
              ))}
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            {[
              ...
              <div key={item.title} className="bg-orange-50 rounded-2xl p-6 border border-orange-100 hover:shadow-lg hover:border-orange-300 transition">
                <div className="text-3xl mb-3">{item.icon}</div>
                <h3 className="font-bold text-stone-800 mb-1 text-base">{item.title}</h3>
      ...
      <section className="py-28 px-8 bg-orange-50">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-16">
            <span className="text-orange-500 font-semibold text-sm uppercase tracking-widest">Who It's For</span>
            <h2 className="text-4xl md:text-5xl font-extrabold text-stone-800 mt-2 mb-4">Made for the moments that matter</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              ...
              <div key={item.title} className="bg-white rounded-2xl p-8 border border-orange-100 hover:shadow-xl hover:border-orange-300 transition group">
                <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-3xl mb-5 ${item.color}`}>
                  {item.icon}
                </div>
                <h3 className="text-xl font-extrabold text-stone-800 mb-3">{item.title}</h3>
                <p className="text-stone-500 text-sm leading-relaxed">{item.description}</p>
              </div>
            ))}
          </div>
      ...
      <section id="features" className="py-28 px-8 bg-white">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-16">
            ...
            <h2 className="text-4xl md:text-5xl font-extrabold text-stone-800 mt-2 mb-4">Built for real people, not tech experts</h2>
            <p className="text-stone-500 text-lg max-w-xl mx-auto">Everything you need. Nothing you don't.</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              { icon: '📋', title: 'Approved Contacts', description: 'Only the people you add can call in or be called. Everyone else is politely blocked — automatically, every time.', color: 'bg-orange-100 text-orange-600' },
              ...
              { icon: '⚡', title: 'Quick Dial', description: 'Press 1 for Mom, Press 2 for Grandma. Assign speed dial slots so calling is always just one button away.', color: 'bg-amber-100 text-amber-700' },
              ...
              <div key={feature.title} className="group p-8 rounded-2xl border border-orange-100 hover:border-orange-300 hover:shadow-xl transition-all duration-300 bg-orange-50">
                <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-2xl mb-5 ${feature.color}`}>
                  {feature.icon}
                </div>
                <h3 className="text-lg font-extrabold text-stone-800 mb-2">{feature.title}</h3>
      ...
      {/* How It Works */}
      <section id="how" className="py-28 px-8 bg-orange-50">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-16">
            <span className="text-orange-500 font-semibold text-sm uppercase tracking-widest">How It Works</span>
            <h2 className="text-4xl md:text-5xl font-extrabold text-stone-800 mt-2 mb-4">Ready in three simple steps</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              { step: '01', title: 'Plug in your phone', description: 'Connect any analog home phone to our pre-configured adapter. It arrives ready to go — no setup needed on your end.', icon: '🔌', color: 'bg-orange-500' },
              ...
              { step: '03', title: 'Share & let them ring!', description: "Your phone is live. Share Ring Ring Club with your circle so they can join too. Approved contacts call freely — everyone else is blocked.", icon: '🔔', color: 'bg-green-400' },
            ].map((item, i) => (
              <div key={item.step} className="relative">
                {i < 2 && (
                  <div className="hidden md:block absolute top-10 left-full w-full h-0.5 bg-gradient-to-r from-orange-200 to-transparent z-0" />
                )}
                <div className="relative bg-white rounded-2xl p-8 border border-orange-100 hover:border-orange-300 hover:shadow-xl transition">
                  <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-3xl mb-5 ${item.color}`}>
                    {item.icon}
                  </div>
                  <div className="text-orange-400 font-black text-sm mb-2 tracking-widest">{item.step}</div>
                  <h3 className="text-xl font-extrabold text-stone-800 mb-3">{item.title}</h3>
                  <p className="text-stone-500 text-sm leading-relaxed">{item.description}</p>
                </div>
              </div>
      ...
      <section className="py-28 px-8 bg-white">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-16">
            ...
            <h2 className="text-4xl font-extrabold text-stone-800 mt-2 mb-4">What families are saying</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              ...
              <div key={t.name} className="bg-orange-50 rounded-2xl p-8 border border-orange-100 hover:shadow-lg hover:border-orange-200 transition">
                <div className="text-orange-400 text-2xl mb-4">★★★★★</div>
                <p className="text-stone-600 text-sm leading-relaxed mb-6 italic">"{t.quote}"</p>
                <div>
                  <p className="font-extrabold text-stone-800 text-sm">{t.name}</p>
      ...
      {/* Pricing */}
      <section id="pricing" className="py-28 px-8 bg-orange-50">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-16">
            <span className="text-orange-500 font-semibold text-sm uppercase tracking-widest">Pricing</span>
            <h2 className="text-4xl md:text-5xl font-extrabold text-stone-800 mt-2 mb-4">Simple, honest pricing</h2>
            ...
            {[
              ...
              <div
                ...
                className={`rounded-2xl p-8 border-2 transition ${
                  plan.highlight
                    ? 'border-orange-400 bg-orange-500 text-white shadow-2xl scale-105'
                    : 'border-orange-100 bg-white hover:border-orange-300 hover:shadow-lg'
                }`}
              >
                {plan.highlight && (
                  <div className="inline-block bg-white/20 text-white text-xs font-extrabold px-3 py-1 rounded-full mb-4 uppercase tracking-widest">
                    Most Popular
                  </div>
                )}
                <div className={`text-sm font-semibold mb-1 ${plan.highlight ? 'text-orange-100' : 'text-stone-400'}`}>{plan.name}</div>
                <div className="flex items-end gap-1 mb-1">
                  <span className={`text-5xl font-extrabold ${plan.highlight ? 'text-white' : 'text-stone-800'}`}>{plan.price}</span>
                  <span className={`text-sm mb-2 ${plan.highlight ? 'text-orange-100' : 'text-stone-400'}`}>/{plan.period}</span>
                </div>
                <p className={`text-sm mb-8 ${plan.highlight ? 'text-orange-100' : 'text-stone-500'}`}>{plan.description}</p>
                <ul className="space-y-3 mb-8">
                  {plan.features.map((f) => (
                    <li key={f} className="flex items-center gap-3 text-sm">
                      <span className={`w-5 h-5 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 ${plan.highlight ? 'bg-white/20 text-white' : 'bg-orange-100 text-orange-600'}`}>✓</span>
      ...
      {/* CTA */}
      <section className="py-28 px-8 bg-orange-500 relative overflow-hidden">
        <div className="absolute inset-0">
          <div className="absolute top-0 left-1/4 w-96 h-96 bg-orange-400 rounded-full blur-3xl opacity-50" />
          <div className="absolute bottom-0 right-1/4 w-96 h-96 bg-amber-400 rounded-full blur-3xl opacity-30" />
        </div>
        <div className="relative max-w-3xl mx-auto text-center">
          <div className="text-6xl mb-6">🔔</div>
          <h2 className="text-4xl md:text-5xl font-extrabold text-white mb-4">
            The phone club your<br />
            <span className="text-orange-100">friends & family deserve.</span>
          </h2>
          ...
          <button
            onClick={() => router.push('/login')}
            className="px-12 py-5 bg-white text-orange-500 text-lg font-extrabold rounded-xl hover:bg-orange-50 transition shadow-2xl"
          >
            Join Ring Ring Club 🔔
          </button>
          <p className="mt-4 text-orange-100 text-sm">No credit card required · Cancel anytime · Share with your circle</p>
        </div>
      </section>
