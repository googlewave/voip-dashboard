...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-white font-sans">
      ...
      {/* Sticky Nav */}
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'bg-indigo-700 shadow-lg py-4' : 'bg-indigo-600 py-6'}`}>
        <div className="max-w-6xl mx-auto px-8 flex items-center justify-between">
          <div className="text-2xl font-extrabold text-white tracking-tight">🔔 Ring Ring Club</div>
          <div className="hidden md:flex items-center gap-8 text-sm font-medium text-indigo-200">
          ...
          <div className="flex items-center gap-3">
            <button onClick={() => router.push('/login')} className="text-indigo-200 hover:text-white font-medium text-sm transition">Sign In</button>
            <button onClick={() => router.push('/login')} className="px-5 py-2.5 bg-yellow-400 text-indigo-900 text-sm font-extrabold rounded-lg hover:bg-yellow-300 transition shadow-md">
              Join the Club 🔔
            </button>
          </div>
        </div>
      ...
      {/* Hero */}
      <section className="relative pt-36 pb-0 bg-indigo-600 overflow-hidden">
        <div className="absolute inset-0 overflow-hidden">
          <div className="absolute -top-20 -right-20 w-96 h-96 bg-indigo-500 rounded-full opacity-50 blur-3xl" />
          <div className="absolute bottom-0 -left-20 w-96 h-96 bg-violet-600 rounded-full opacity-40 blur-3xl" />
        </div>

        <div className="relative max-w-6xl mx-auto px-8 grid grid-cols-1 lg:grid-cols-2 gap-12 items-end">
          {/* Left: Copy */}
          <div className="pb-20">
            <div className="inline-flex items-center gap-2 bg-white/10 text-indigo-100 text-sm font-semibold px-4 py-2 rounded-full mb-8 border border-white/20">
            ...
            <h1 className="text-5xl md:text-6xl font-extrabold text-white mb-6 leading-tight tracking-tight">
              ...
              <span className="text-yellow-400">friends & family deserve.</span>
            </h1>
            <p className="text-indigo-200 text-xl mb-10 leading-relaxed max-w-lg">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4">
              <button
                ...
                className="w-full sm:w-auto px-10 py-4 bg-yellow-400 text-indigo-900 text-lg font-extrabold rounded-xl hover:bg-yellow-300 transition shadow-xl"
              >
                Join the Club 🔔
              </button>
              <button
            ...
            <p className="mt-6 text-indigo-300 text-sm">No credit card required · Set up in minutes · Share with your circle</p>
          </div>
            ...
            <div className="absolute bottom-8 -left-4 bg-white rounded-2xl shadow-2xl p-4 border border-stone-100">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm font-bold text-stone-800">Mom called Emma ✓</p>
                </div>
              </div>
            </div>
            {/* Floating badge right */}
            <div className="absolute top-8 -right-4 bg-white rounded-2xl shadow-2xl p-4 border border-stone-100">
              <div className="flex items-center gap-3">
      ...
      {/* Social Proof Bar */}
      <section className="py-10 bg-indigo-900 border-b border-indigo-800">
        <div className="max-w-6xl mx-auto px-8 text-center">
          <p className="text-indigo-300 text-sm font-semibold uppercase tracking-widest mb-6">Families & friend groups sharing the club love</p>
          <div className="flex flex-wrap items-center justify-center gap-10 text-indigo-500 font-bold text-base">
            {['The Johnsons', 'Smith Family', 'The Garcias', 'The Williams', 'The Nguyens', 'Team Rodriguez'].map((name) => (
              <span key={name} className="hover:text-indigo-300 transition cursor-default">{name}</span>
            ))}
          </div>
        </div>
      </section>
      ...
      <section className="py-28 px-8 bg-white">
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div>
            <span className="text-indigo-600 font-semibold text-sm uppercase tracking-widest">Your Circle. Your Club.</span>
            ...
            <div className="flex flex-wrap gap-4">
              {['Share with neighbors', 'Invite your family', 'Grow your circle', 'Keep it local'].map((tag) => (
                <span key={tag} className="px-4 py-2 bg-indigo-50 text-indigo-700 font-semibold text-sm rounded-full border border-indigo-100">{tag}</span>
              ))}
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            {[
              ...
              <div key={item.title} className="bg-indigo-50 rounded-2xl p-6 border border-indigo-100 hover:shadow-lg hover:border-indigo-300 transition">
      ...
      {/* Use Cases */}
      <section className="py-28 px-8 bg-indigo-600">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-16">
            <span className="text-yellow-400 font-semibold text-sm uppercase tracking-widest">Who It's For</span>
            <h2 className="text-4xl md:text-5xl font-extrabold text-white mt-2 mb-4">Made for the moments that matter</h2>
            <p className="text-indigo-200 text-lg max-w-xl mx-auto">No matter where your people are, Ring Ring Club keeps you close.</p>
          </div>
            ...
            {[
              {
                ...
                color: 'bg-yellow-400 text-indigo-900',
              },
              {
                icon: '👵',
              ...
              <div key={item.title} className="bg-white/10 backdrop-blur rounded-2xl p-8 border border-white/20 hover:bg-white/20 transition group">
      ...
      <section id="features" className="py-28 px-8 bg-white">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              { icon: '📋', title: 'Approved Contacts', description: 'Only the people you add can call in or be called. Everyone else is politely blocked — automatically, every time.', color: 'bg-indigo-100 text-indigo-700' },
              ...
              { icon: '🌍', title: 'International Calling', description: 'Keep family ties strong no matter where they live. Call loved ones abroad as easily as calling next door.', color: 'bg-pink-100 text-pink-600' },
              { icon: '🔒', title: 'Private & Secure', description: 'Your account is completely private. Only you can see your devices and contacts. Always and forever.', color: 'bg-violet-100 text-violet-600' },
            ].map((feature) => (
              <div key={feature.title} className="group p-8 rounded-2xl border-2 border-stone-100 hover:border-indigo-300 hover:shadow-xl transition-all duration-300 bg-white">
      ...
      {/* How It Works */}
      <section id="how" className="py-28 px-8 bg-indigo-50">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              { step: '01', title: 'Plug in your phone', description: 'Connect any analog home phone to our pre-configured adapter. It arrives ready to go — no setup needed on your end.', icon: '🔌', color: 'bg-indigo-600' },
              { step: '02', title: 'Add your circle', description: 'Log in to your Ring Ring Club dashboard and add the friends and family members you want to allow. Just a name and number.', icon: '👨‍👩‍👧', color: 'bg-yellow-400' },
              ...
              <div key={item.step} className="relative">
                {i < 2 && (
                  <div className="hidden md:block absolute top-10 left-full w-full h-0.5 bg-gradient-to-r from-indigo-200 to-transparent z-0" />
                )}
                <div className="relative bg-white rounded-2xl p-8 border-2 border-stone-100 hover:border-indigo-300 hover:shadow-xl transition">
                  <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-3xl mb-5 ${item.color}`}>
                    {item.icon}
                  </div>
                  <div className="text-indigo-400 font-black text-sm mb-2 tracking-widest">{item.step}</div>
      ...
      {/* Testimonials */}
      <section className="py-28 px-8 bg-indigo-900">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              ...
              <div key={t.name} className="bg-white/10 backdrop-blur rounded-2xl p-8 border border-white/10 hover:bg-white/20 transition">
                ...
                <p className="text-indigo-100 text-sm leading-relaxed mb-6 italic">"{t.quote}"</p>
                <div>
                  <p className="font-extrabold text-white text-sm">{t.name}</p>
                  <p className="text-indigo-400 text-xs">{t.location}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section id="pricing" className="py-28 px-8 bg-white">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-16">
            <span className="text-indigo-600 font-semibold text-sm uppercase tracking-widest">Pricing</span>
            ...
            {[
              ...
              <div
                ...
                className={`rounded-2xl p-8 border-2 transition ${
                  plan.highlight
                    ? 'border-yellow-400 bg-indigo-600 text-white shadow-2xl scale-105'
                    : 'border-stone-200 bg-white hover:border-indigo-300 hover:shadow-lg'
                }`}
              >
                {plan.highlight && (
                  <div className="inline-block bg-yellow-400/20 text-yellow-300 text-xs font-extrabold px-3 py-1 rounded-full mb-4 uppercase tracking-widest">
                    Most Popular
                  </div>
                )}
                <div className={`text-sm font-semibold mb-1 ${plan.highlight ? 'text-indigo-200' : 'text-stone-400'}`}>{plan.name}</div>
                <div className="flex items-end gap-1 mb-1">
                  <span className={`text-5xl font-extrabold ${plan.highlight ? 'text-white' : 'text-stone-900'}`}>{plan.price}</span>
                  <span className={`text-sm mb-2 ${plan.highlight ? 'text-indigo-200' : 'text-stone-400'}`}>/{plan.period}</span>
                </div>
                <p className={`text-sm mb-8 ${plan.highlight ? 'text-indigo-200' : 'text-stone-500'}`}>{plan.description}</p>
                <ul className="space-y-3 mb-8">
                  {plan.features.map((f) => (
                    <li key={f} className="flex items-center gap-3 text-sm">
                      <span className={`w-5 h-5 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 ${plan.highlight ? 'bg-yellow-400/20 text-yellow-300' : 'bg-indigo-100 text-indigo-600'}`}>✓</span>
                      <span className={plan.highlight ? 'text-indigo-100' : 'text-stone-600'}>{f}</span>
                    </li>
                  ))}
                </ul>
                <button
      ...
      {/* CTA */}
      <section className="py-28 px-8 bg-indigo-600 relative overflow-hidden">
        <div className="absolute inset-0">
          <div className="absolute top-0 left-1/4 w-96 h-96 bg-indigo-500 rounded-full blur-3xl opacity-60" />
          <div className="absolute bottom-0 right-1/4 w-96 h-96 bg-violet-600 rounded-full blur-3xl opacity-40" />
        </div>
        <div className="relative max-w-3xl mx-auto text-center">
          ...
          <button
            onClick={() => router.push('/login')}
            className="px-12 py-5 bg-yellow-400 text-indigo-900 text-lg font-extrabold rounded-xl hover:bg-yellow-300 transition shadow-2xl"
          >
            Join Ring Ring Club 🔔
          </button>
          <p className="mt-4 text-indigo-300 text-sm">No credit card required · Cancel anytime · Share with your circle</p>
        </div>
      </section>

      {/* Footer */}
      <footer className="bg-indigo-900 text-indigo-400 py-16 px-8">
        <div className="max-w-6xl mx-auto">
          <div className="flex flex-col md:flex-row items-start justify-between gap-8 mb-12">
            <div>
              <div className="text-2xl font-extrabold text-white mb-2">🔔 Ring Ring Club</div>
              <p className="text-sm text-indigo-400 max-w-xs">The phone club your friends and family deserve. Simple, safe, and human.</p>
            </div>
          ...
          <div className="border-t border-indigo-800 pt-8 flex flex-col md:flex-row items-center justify-between gap-4 text-sm">
